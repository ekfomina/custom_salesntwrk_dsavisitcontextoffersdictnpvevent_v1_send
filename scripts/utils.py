    ...
    def emit(self, record):
        ...
        try:
            ...
            if not response.ok:
                ...
                rs = response.reason
                self.logger.warning(f"logger response status code {sc}: {rs}")
    ...
    def emit(self, record):
        ...
        try:
            ...
            if not response.ok:
                sc = response.status_code
                rs = response.reason
                logging.warning(f"logger response status code {sc}: {rs}")
...
def exception_hook(exc_type, exc_value, exc_traceback):
    ...
    logging.error(msg=f"{exc_value}\n{fmt_stacktrace}",
                  exc_info=(exc_type, exc_value, exc_traceback))

    return None


def load_schema(path_to_file: str) -> dict:
    """
    получает из файла схему для валидации json-а

    Args:
        path_to_file: путь до файла со схемой

    Returns: схема необходимая для валидации (python dict)
